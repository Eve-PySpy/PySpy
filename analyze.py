...
zKIllboard's API, to be stored in a temporary in-memeory SQLite3 database.
'''
# **********************************************************************
import logging
import json
...
def get_affil_names(conn, cur):
    ...
    statusmsg.push_status("Obtaining corporation and alliance names...")
    try:
...
class zKillStats(threading.Thread):
    ...
    def run(self):
        ...
        threads = []
        q_sub = queue.Queue()
        statusmsg.push_status("Getting zKillboard stats for first " + str(max) + " characters in list.")
